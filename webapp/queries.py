...
def get_labels_continent_with_continent_code(continent_code='All'):
    ...
    if continent_code!='All':
        continents = dp.transformed_raw_data.loc[:, ['Continent', 'continent_code']].drop_duplicates()
        label = continents.loc[continents['continent_code']==continent_code]['Continent'].values[0]
    else:
...
def get_labels_statements(statement_id=None):
    if statement_id==None:
        ...
        statements = dp.transformed_raw_data.loc[:, ['Statement', 'statement_id']].drop_duplicates()
        label = statements.loc[statements['statement_id']==statement_id]['Statement'].values[0]
